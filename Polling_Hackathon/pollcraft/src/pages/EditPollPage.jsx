...
import { Plus, Trash2, GripVertical, ChevronDown, ChevronUp, ArrowLeft, Globe, Users, Lock, Calendar } from 'lucide-react'

const makeOption = () => ({ id: Math.random().toString(36).slice(2, 10), text: '' })
const makeQuestion = () => ({
...
const VISIBILITY_OPTS = [
  ...
  { val: 'respondents', label: 'Respondents', desc: 'Only people who voted', Icon: Users, color: 'text-azure', border: '[border-color:var(--azure-border)]', bg: '[background:var(--azure-dim)]' },
...
function EditPollContent() {
  ...
  useEffect(() => {
    async function loadPoll() {
      try {
        ...
        setAnonymousAllowed(poll.anonymousAllowed || false)
        setResultsVisibility(poll.resultsVisibility || 'all')
        setExpiresAt(poll.expiresAt ? poll.expiresAt.slice(0, 16) : '')
        setPublishOn(poll.publishOn ? poll.publishOn.slice(0, 16) : '')
        if (poll.questions?.length) {
          const mapped = poll.questions.map(q => ({
            _key: q.id,
            id: q.id,
            content: q.content,
          ...
          setQuestions(mapped)
          const expanded = {}
          mapped.forEach((q, i) => { if (i === 0) expanded[q._key] = true })
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        publishOn: toISO(publishOn) || null,
        resultsVisibility,
      ...
      await pollsApi.update(id, payload)
  ...
  if (fetching) {
    return (
      <div className="min-h-screen pt-14 flex items-center justify-center" style={{ background: 'var(--page-bg)' }}>
        <div className="flex gap-1">
          {[0,1,2].map(i => (
            <span key={i} className="w-2 h-2 rounded-full animate-pulse" style={{ background: 'var(--signal)', animationDelay: `${i*0.15}s` }} />
          ))}
        </div>
      </div>
    )
    ...
    <div className="min-h-screen pt-14" style={{ background: 'var(--page-bg)' }}>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="font-display text-4xl tracking-wider" style={{ color: 'var(--text-primary)' }}>EDIT POLL</h1>
        </div>
            ...
            <div>
              ...
              <div className="flex gap-2 pt-1">
                {[
                  ...
                  <button
                    key={String(opt.val)}
                    onClick={() => setAnonymousAllowed(opt.val)}
